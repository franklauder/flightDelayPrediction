...
Gradient Boost is another example of ensemble learning though instead of using majority rules like Random Forest, it uses incorrect predictions from previous trees to adjust the next tree. This is accomplished by fitting each new tree based on the errors of the previous tree’s predictions. Mistakes from the previous trees are used to build a new tree solely around these mistakes.
...
gb_cv_model = gb_cv.fit(flights_train)
...
gb_model = gb_cv_model.bestModel
...
gb_cv_avg_mwetrics=gb_cv_model.avgMetrics[0]
...
print(f"Average Gradient Boost Cross Validation Score: {gb_cv_avg_mwetrics:.2f}")
...
gb_train_preds = gb_model.transform(flights_train)
...
gb_train_accuracy = (
    accuracy_eval.evaluate(gb_train_preds)) * 100
...
precision_eval.setMetricLabel(1)
...
gb_precision_train_label_1=precision_eval.evaluate(gb_train_preds)
...
gb_recall_train_label_1=recall_eval.evaluate(gb_train_preds)
...
gb_f1_train_label_1=f1_eval.evaluate(gb_train_preds)
...
precision_eval.setMetricLabel(0)
recall_eval.setMetricLabel(0)
f1_eval.setMetricLabel(0)
...
gb_precision_train_label_0=precision_eval.evaluate(gb_train_preds)
...
gb_recall_train_label_0=recall_eval.evaluate(gb_train_preds)
...
gb_f1_train_label_0=f1_eval.evaluate(gb_train_preds)
...
print("Gradient Boost Train Accuracy Score: {0:.2f}".format(gb_train_accuracy))
...
print("Gradient Boost Train Precision Label 1: {0:.2f}".format(gb_precision_train_label_1))
print("Gradient Boost Train Recall Label 1: {0:.2f}".format(gb_recall_train_label_1))
print("Gradient Boost Train F1 Score Label 1: {0:.2f}".format(gb_f1_train_label_1))
...
print("Gradient Boost Train Precision Label 0: {0:.2f}".format(gb_precision_train_label_0))
print("Gradient Boost Train Recall Label 0: {0:.2f}".format(gb_recall_train_label_0))
print("Gradient Boost Train F1 Score Label 0: {0:.2f}".format(gb_f1_train_label_0))
...
gb_auprc_train = evaluator.evaluate(gb_train_preds,
                           {evaluator.metricName: "areaUnderPR"})
print("Gradient Boost Area Under Precision-Recall Curve Score-Training: {:.3f}".format(gb_auprc_train))
...
gb_prob_df=gb_train_preds.select("label", "probability")
...
gb_train_auroc = evaluator.evaluate(gb_train_preds,
                           {evaluator.metricName: "areaUnderROC"})
...
print("Gradient Boost Area Under Roc Curve Score-Training: {:.3f}".format(gb_train_auroc))
...
train_predictions_and_labels = gb_train_preds.select(
    "prediction", "label").rdd.map(
        lambda row: (float(row["prediction"]), float(row["label"])))
...
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.title("Confusion Matrix Gradient Boost (Train Set)")
...
gb_test_pred = gb_model.transform(flights_test)
...
gb_test_pred.select("label", "prediction", "probability", "features").show(10)
...
gb_test_accuracy = accuracy_eval.evaluate(gb_test_pred) * 100
...
gb_precision_test_label_1=precision_eval.evaluate(gb_test_pred)
...
gb_recall_test_label_1=recall_eval.evaluate(gb_test_pred)
...
gb_f1_test_label_1=f1_eval.evaluate(gb_test_pred)
...
gb_precision_test_label_0=precision_eval.evaluate(gb_test_pred)
...
gb_recall_test_label_0=recall_eval.evaluate(gb_test_pred)
...
gb_f1_test_label_0=f1_eval.evaluate(gb_test_pred)
...
print("Gradient Boost Test  Accuracy: {0:.2f}".format(gb_test_accuracy), "%")
...
print( f"Gradient Boost Test Precision label 1: {gb_precision_test_label_1:.2f}")
print(f"Gradient Boost Test Precision label 0: {gb_precision_test_label_0:.2f}")
...
print(f"Gradient Boost Test Recall label 1: {gb_recall_test_label_1:.2f}")
print(f"Gradient Boost Test Recall label 0: {gb_recall_test_label_0:.2f}")
...
print(f"Gradient Boost Test F1 label 1: {gb_f1_test_label_1:.2f}")
print(f"Gradient Boost Test F1 label 0: {gb_f1_test_label_0:.2f}")
...
gb_test_auroc = evaluator.evaluate(gb_test_pred,
                                   {evaluator.metricName: "areaUnderROC"})
...
gb_test_aupr = evaluator.evaluate(gb_test_pred,
                                   {evaluator.metricName: "areaUnderPR"})
...
print("Gradient Boost Test Area under ROC Curve: {:.3f}".format(gb_test_auroc))
print("Gradient Boost Test Area under PR Curve: {:.3f}".format(gb_test_aupr))
...
gb_featureImportances = gb_model.featureImportances.toArray()
...
feature_list = extract_feature_imp(gb_featureImportances, final_data,
                                   "features")
top_20_features = feature_list.sort_values('score', ascending=False).head(20)
